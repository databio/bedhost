...
const TooltipContent = ({ tooltip, showLink, simpleTooltip }: { tooltip: TooltipProps['tooltip']; showLink?: boolean; simpleTooltip?: boolean; }) => {
  if (!tooltip) return null;
  return (
    <div
      className='border rounded text-xs overflow-hidden fade-in-fast'
      style={{
        maxWidth: '300px',
        pointerEvents: 'none',
    ...
    >
      {simpleTooltip ? (
        <div className='p-2'>
          <div className='fw-bold text-center'>You are here!</div>
        </div>
      ) : (
        <div className='p-2'>
...
export class AtlasTooltip {
  ...
  update(props: TooltipProps) {
    ...
    this.root.render(<TooltipContent tooltip={props.tooltip} showLink={this.showLink} simpleTooltip={this.simpleTooltip}/>);
  }
